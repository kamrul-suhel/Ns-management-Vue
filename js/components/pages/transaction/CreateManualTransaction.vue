...
<script>
    import ManuelProductLoopComponent from './partials/ManuelProductLoopComponent';
    import TransactionEventBus from '../../../event_bus/transaction_event';

    export default {
        components: {
            'productComponent': ManuelProductLoopComponent
        },
        ...
        methods: {
            async initialize() {
                ...
                //get all product
                axios.get('/api/products?status=available&allSerial=true')
                    .then((response) => {
                        if (response.data.products) {
                            this.products = response.data.products;
                            this.allProductData = [...response.data.products];
                            this.$store.commit('setAvailableProducts', response.data.products)
                            let array_products = [];
